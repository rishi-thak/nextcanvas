...
export function applyBoundTextEdit(edit: Edit): EditResult {
  ...
  if (collectionName) {
    ...
    // Value-match targeting: pick the array entry whose bound property CURRENTLY
    ...
    if (matches.length === 1) {
      ...
      return {
        ok: false,
        error: `"${oldText}" appears ${matches.length}× in ${collectionName}; nextcanvas can't tell which entry you meant. Make the value unique, or edit it directly in the data file.`,
      };
    } else {
      return {
        ok: false,
        error:
          leafErr ??
          `No entry in ${collectionName} currently has ${propPath.join('.')} === "${oldText}"; reload and try again.`,
      };
    }
  } else {
